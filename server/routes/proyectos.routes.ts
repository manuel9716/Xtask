...
proyectosRouter.use(authRequired);

// Esquema de validación para asignar empleados a proyectos
const teamAssignSchema = z.object({
  employeeId: z.number(),
  role: z.enum(['member', 'lead', 'manager']).default('member')
});

// Esquema para establecer el responsable principal (manager)
...
// GET /api/projects/:id/team - Obtener equipo de un proyecto
proyectosRouter.get('/:id/team', async (req: Request, res: Response) => {
  try {
    ...
    // Obtener asignaciones para este proyecto
    const assignments = await db
      .select({
        ...
        assignedBy: employeeProjects.assignedBy,
        isActive: employeeProjects.isActive
      })
    ...
    // Si hay asignaciones, obtener información de los empleados
    if (assignments.length > 0) {
      ...
      const employeesData = await db
        .select()
        .from(employees)
        .where(sql`${employees.id} IN (${employeeIds.join(',')})`);
      
      // Asociar información de empleados a cada asignación
      const result = assignments.map(assignment => {
        const employee = employeesData.find(e => e.id === assignment.employeeId);
        return {
          ...assignment,
          employee,
          isPrimary: project.managerId === assignment.employeeId
        };
      });
...
// POST /api/projects/:id/team - Asignar empleado a un proyecto
proyectosRouter.post('/:id/team', async (req: Request, res: Response) => {
  try {
    const projectId = Number(req.params.id);
    if (isNaN(projectId)) {
      return res.status(400).json({ error: 'ID de proyecto inválido' });
    ...
    // Validar datos
    const { employeeId, role } = teamAssignSchema.parse(req.body);
    ...
    if (existingAssignment) {
      return res.status(400).json({ error: 'El empleado ya está asignado a este proyecto' });
    }
    
    // Obtener el ID del usuario actual (simplificado para el prototipo)
...
// POST /api/projects/:id/manager - Establecer responsable principal del proyecto
proyectosRouter.post('/:id/manager', async (req: Request, res: Response) => {
  try {
    const projectId = Number(req.params.id);
    if (isNaN(projectId)) {
      return res.status(400).json({ error: 'ID de proyecto inválido' });
    ...
    // Verificar que el empleado existe
    const [employee] = await db.select().from(employees).where(eq(employees.id, managerId)).limit(1);
    ...
    const [existingAssignment] = await db
      ...
      .from(employeeProjects)
      .where(and(
        eq(employeeProjects.employeeId, managerId),
        eq(employeeProjects.projectId, projectId),
        eq(employeeProjects.isActive, true)
      ))
      .limit(1);
    ...
    if (!existingAssignment) {
      // Si no está asignado, primero lo asignamos
      const currentUserId = req.user?.id || 1;
    ...
    } else if (existingAssignment.role !== 'manager') {
      // Si está asignado pero no como manager, actualizamos su rol
      await db
    ...
    // Actualizar el responsable en el proyecto
    ...
    console.error('Error al establecer responsable del proyecto:', error);
    res.status(500).json({ error: error.message });
  }
});

// DELETE /api/employee-projects/:id - Eliminar asignación de empleado a proyecto
proyectosRouter.delete('/employee-projects/:id', async (req: Request, res: Response) => {
  try {
    const id = Number(req.params.id);
    if (isNaN(id)) {
    ...
    // Verificar si es el responsable principal del proyecto
    const [project] = await db
      .select()
      .from(projects)
      .where(eq(projects.id, existingAssignment.projectId))
      .limit(1);
    
    if (project && project.managerId === existingAssignment.employeeId) {
      return res.status(400).json({ 
        error: 'No se puede eliminar al responsable principal del proyecto. Asigne otro responsable primero.' 
      });
    }
    
    // Eliminar asignación (actualizar isActive a false)
    await db
      .update(employeeProjects)
      .set({ isActive: false })
      .where(eq(employeeProjects.id, id));
