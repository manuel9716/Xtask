...
// Obtener datos para el dashboard de nómina
nominaRouter.get('/v1/dashboard', async (req: Request, res: Response) => {
  try {
    // Contar nóminas por estado
    const nominasPendientes = await db.select({ count: sql<number>`count(*)` })
      .from(nominas)
      .where(eq(nominas.estado, EstadoNomina.PENDIENTE));
    ...
    const nominasRecientes = await db.select()
      .from(nominas)
      .orderBy(sql`${nominas.fechaCreacion} DESC`)
      .limit(5);
    
    // Calcular totales de montos
    const totalPendiente = await db.select({ 
      sum: sql<string>`COALESCE(SUM(CAST(${nominas.montoTotal} AS NUMERIC)), 0)` 
    })
    .from(nominas)
    .where(eq(nominas.estado, EstadoNomina.PENDIENTE));
    
    const totalPagado = await db.select({ 
      sum: sql<string>`COALESCE(SUM(CAST(${nominas.montoTotal} AS NUMERIC)), 0)` 
    })
    .from(nominas)
    .where(eq(nominas.estado, EstadoNomina.PAGADA));
    ...
    // Formar respuesta
    return res.status(200).json({
      nominasPendientes: nominasPendientes[0]?.count || 0,
      nominasPagadas: nominasPagadas[0]?.count || 0,
      nominasCanceladas: nominasCanceladas[0]?.count || 0,
      totalPendiente: parseFloat(totalPendiente[0]?.sum || '0'),
      totalPagado: parseFloat(totalPagado[0]?.sum || '0'),
      empleadosActivos: empleadosActivos[0]?.count || 0,
      nominasRecientes
    });
  } catch (error: any) {
    console.error('Error al obtener datos del dashboard de nómina:', error);
    return res.status(500).json({ 
...
// Listar todas las nóminas (v1)
nominaRouter.get('/v1/listar', async (req: Request, res: Response) => {
  try {
    const page = Number(req.query.page) || 1;
    const pageSize = Number(req.query.pageSize) || 10;
    const offset = (page - 1) * pageSize;
    
    // Obtener todas las nóminas ordenadas por fecha de creación descendente
    const todasLasNominas = await db.select()
      .from(nominas)
      .orderBy(sql`${nominas.fechaCreacion} DESC`)
      .limit(pageSize)
      .offset(offset);
    ...
    return res.status(200).json({
      nominas: todasLasNominas,
      total,
      paginaActual: page,
      totalPaginas
    });
  } catch (error: any) {
    console.error('Error al listar nóminas (v1):', error);
    return res.status(500).json({ 
