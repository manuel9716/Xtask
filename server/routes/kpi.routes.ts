...
kpiRouter.patch("/:id/resultado", isAuthenticated, async (req: Request, res: Response) => {
  try {
    ...
    const [updatedKpi] = await db
      .update(userKpis)
      .set({
        valorObtenido: valorObtenidoNum,
...
kpiRouter.patch("/bonificacion/:id/aprobar", isAuthenticated, async (req: Request, res: Response) => {
  try {
    ...
    const [existingBonificacion] = await db
      .select()
      .from(bonificacionesMensuales)
    ...
    // Validar que el aprobador sea diferente del usuario de la bonificación
    if (existingBonificacion.userId === aprobadorId) {
      return res.status(400).json({ error: "No puedes aprobar tu propia bonificación" });
    }
    ...
    console.error("Error al aprobar bonificación:", error);
    res.status(500).json({ error: error.message });
  }
});
